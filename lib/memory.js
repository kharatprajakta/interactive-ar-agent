...
export async function getHistory(userId, persona, limit = HISTORY_LIMIT) {
  const rows = await many('SELECT role, content FROM messages WHERE user_id = $1 AND persona = $2 ORDER BY id DESC LIMIT $3', [userId, persona, limit]);
  return rows.reverse().map((m) => ({ role: m.role, content: m.content }));
}

export async function addMessage(userId, persona, role, content) {
  await query('INSERT INTO messages (user_id, persona, role, content) VALUES ($1, $2, $3, $4)', [userId, persona, role, content.slice(0, 8000)]);
}
...
export async function forget(userId, { fact, field }) {
  ...
  if (field === 'history') await query('DELETE FROM messages WHERE user_id = $1', [userId]);
