...
export async function ollamaChat(body, signal) {
...
- rejected: true only if the student's LATEST message says the tutor got their class or subject wrong (e.g. "no", "not that one"), otherwise false.`,
  ...
  try {
    ...
    return { class_num: 0, subject: '', chapter: 0, topic: '', rejected: false };
  }
}

async function* ncertContext(messages, signal, saved = null) {
 ...
 */
export async function* respond({ persona, messages, sessionId, userNote = '', study = null, signal }) {
  ...
  if (persona.ncert) {
    try {
      const gen = ncertContext(messages, signal, study);
      let step;
